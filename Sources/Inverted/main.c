...
// Version for inverted LED panels 1.00
//
...
void Timer0Init(void) // 100ms @11.0592MHz
{
...
int main()
{
  ...
  // Set P0.1 (BUZZER) to Open-Drain Mode
  P0M1 |=  (1 << 1); 
  P0M0 |=  (1 << 1);
  ...
  while(1)
  {
    ...
    // Sound chime or alarm every even second
    if (beep && (chimeDuration || alarmDuration))
    {
      if (rtc.seconds % 2 == 0) BUZZER = 0;
      else BUZZER = 1;
    }
